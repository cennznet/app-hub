...
export function getAssetFromCore(
	coreAmount: Amount,
	poolBalance: IExchangePool
) {
	const coreReserve = poolBalance.coreAssetBalance;
	...
	if (coreReserve.isZero() || assetReserve.isZero()) {
		...
		return coreAmount.mul(assetReserve).div(coreReserve).addn(ROUND_UP);
	}
}

export function getCoreFromAsset(
	assetAmount: Amount,
	poolBalance: IExchangePool,
	buffer: number
) {
	...
	if (coreReserve.isZero() || assetReserve.isZero()) {
		...
		return assetAmount.mul(coreReserve).div(assetReserve).subn(ROUND_UP);
